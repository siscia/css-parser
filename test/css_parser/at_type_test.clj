...
(deftest supports-test
  (testing "support condition"
    (is (= (p/parse gp "@supports ( transform-origin: 5% 5% ) { background: red}")
           [:debug [:at-rules [:supports "@supports" [:support-condition "(" " transform-origin" ":" " 5% 5% " ")"] "{" [:declarations [:token [:background "background" [:background-color-value [:color-type [:color-keyword "red"]]]]]] "}"]]]))
    (is (= (p/parse gp "@supports (background:red) and ( transform-origin: 5% 5% ) { background: red}")
           [:debug [:at-rules [:supports "@supports" [:support-condition "(" "background" ":" "red" ")" "and" "(" " transform-origin" ":" " 5% 5% " ")"] "{" [:declarations [:token [:background "background" [:background-color-value [:color-type [:color-keyword "red"]]]]]] "}"]]]))))
