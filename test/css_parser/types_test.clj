(ns css-parser.types-test
  (:require [clojure.test :refer [deftest testing is]]
            [instaparse.core :as p]))

(def gp (p/parser (slurp (clojure.java.io/resource "types.bnf"))))
...
(deftest image-test
  (testing "image, not that an image is either an url, a gradient or an element, an element is not a css type"
    ...
    (is (= (p/parse gp "element(#colonne3)")
           [:type [:image [:element "element(" "#" "colonne3" ")"]]]))))
...
[:type [:count [:counter "counter(" [:identifier [:custom-ident "s" "ection" ""]] ")"]]]))
    (is (= [:type [:count [:counter "counter(" [:identifier [:custom-ident "s" "ection" ""]] "," [:style "decimal"] ")"]]])))
  (testing "counters"
